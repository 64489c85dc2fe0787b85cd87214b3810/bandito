<script lang="ts" context="module">
  ...
  const [_, bem] = createNamespace('box');
</script>

<script lang="ts">
  import './Box.scss';
  import { type BoxProps } from './types';
  import { getResponsiveProps, cssStringify, sanitizeCustomProperties } from './utils';
  ...
  let component = $derived(as);
</script>

<svelte:element
  this={component}
  style={cssStringify(sanitizeCustomProperties(style))}
  {id}
  {role}
  tabindex={tabIndex}
  class={bem([visuallyHidden, printHidden, { listReset: as === 'ul' }])}
  {...restProps}
  >{#if children}
    {@render children()}
  {/if}
